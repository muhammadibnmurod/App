<template>
  <div class="w-[260px] shrink-0 h-screen flex flex-col gap-6 p-4">
    <button
      class="h-11 flex items-center gap-2 rounded-lg py-1.5 px-2 border border-surface-40"
    >
      <div
    ...
    <div
      class="border border-surface-30 rounded-xl py-2.5 px-2 bg-surface-40 flex items-center gap-2"
    >
      <div>
...
const systems = computed<number>(
  () => permissionStore.userInfo?.accountRole.systems ?? 0
);
...
const renderDropdownOption = ({ node, option }: any) => {
  ...
  return h(
    "div",
    {
      ...
      // onClick o'chirildi - Naive UI o'zi handle qiladi
    },
    [
...
// Naive UI ning @select eventidan foydalanish
const handleSelectFromNaive = (key: string) => {
  handleSelect(key);
  showDropdown.value = false; // Dropdownni yopish
};
