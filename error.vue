    ...
    <div
      ...
      <div class="flex items-center justify-center flex-col gap-10">
        <div class="flex flex-col justify-center text-center gap-1">
          <span class="text-[#0A0F1A] font-bold text-[30px]">{{ $t('error.pageNotFound') }}</span>
...
const firstAllowedRoute = computed(() => {
  for (const p of platformRoutes) {
    console.log('Checking permission for route:', p.route, systems, 'with permission:', p.permission, hasPermission(systems, p.permission));
